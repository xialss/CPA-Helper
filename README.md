...
- **Response model inspection**: Display CPA's reported response model separately from the usage model without changing billing. Only super administrators can explicitly retrieve full CPA request logs and inspect response/signature model declarations. This requires available CPA request logs; Helper does not enable logging or collect full logs automatically, and stores only audit summaries.
