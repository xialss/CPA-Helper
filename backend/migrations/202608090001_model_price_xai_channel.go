...
func init() {
	goose.AddMigrationNoTxContext(upModelPriceXAIChannel, nil)
}

func upModelPriceXAIChannel(ctx context.Context, db *sql.DB) (err error) {
		...
		CREATE TABLE "__goose_model_prices" (
			...
			CONSTRAINT ck_model_prices_scope CHECK (
				...
				(price_scope = 'channel'
					...
					AND channel_brand IN ('gemini', 'codex', 'claude', 'openai_compatibility', 'vertex', 'xai')
					AND (channel_auth_type <> 'oauth' OR channel_brand NOT IN ('openai_compatibility', 'xai'))
	...
	if err := execStatements(ctx, tx,
		...
		`CREATE UNIQUE INDEX uq_model_prices_native_channel_model
		 ...
		 WHERE price_scope = 'channel' AND channel_brand IN ('gemini', 'codex', 'claude', 'vertex', 'xai')`,
