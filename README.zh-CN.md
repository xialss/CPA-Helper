...
- **响应模型核查**：独立展示 CPA 上报的响应模型，不改变现有计费。仅超级管理员可主动获取完整 CPA 请求日志并核查响应／签名中的模型声明；需要 CPA 日志可用。Helper 不会自动开启日志或全量采集日志，仅保存核查摘要。
- **用户与权限管理**：支持超级管理员、管理员和普通用户视图；管理员可创建、禁用普通用户账号，并维护用户昵称、登录账号、密码和角色，只有超级管理员可以修改余额。
- **用户余额与 Key 自动暂停**：用户默认不限制余额；超级管理员可设置每月余额和不限时余额，用量按当前模型价格折算为 USD，优先扣每月余额，不足再扣不限时余额，余额耗尽后只暂停该用户的 CPA API Key。
...
管理员可创建或禁用普通用户账号，维护昵称、角色、启停状态，并查看用户维度的今日用量、总费用、每月余额和不限时余额；只有超级管理员可以修改余额。
...
查看 AI.INPUT.IM 上游分钟样本，以及 OpenAI、Claude Status 和 DeepSeek 官方日级状态历史。管理员可启用或停用这四个内置来源并调整显示顺序；暂不支持自定义第三方状态页。
...
- 所有现有用户和新建用户默认“不限制余额”。
- 超级管理员可在“用户管理”里关闭不限制开关，并设置每月余额与不限时余额；两个数值默认 `0`，且不能留空。普通管理员可以查看余额，但不能修改。
...
- 每月余额按 `Asia/Shanghai` 自然月重置。进入新月份、超级管理员补充余额或重新打开不限制余额后，仅因余额耗尽暂停的 Key 会自动恢复。
...
- “模型监控”页面独立采集 AI.INPUT.IM、OpenAI、Claude Status 和 DeepSeek，保留各来源原生的分钟或日级历史。它不会发送付费模型探测，也不代表特定账号或中转线路的端到端可用性。管理员可启用、停用和排序这四个内置来源，并可单独刷新某个来源或刷新全部已启用来源；直连不可用时还可配置独立代理。暂不支持自定义第三方状态页。
