...
查看 AI.INPUT.IM 上游分钟样本，以及 OpenAI 与 Claude Status 官方日级状态历史，来源固定为这三个内置状态页。
...
- “模型监控”页面独立采集 AI.INPUT.IM、OpenAI 与 Claude Status，保留各来源原生的分钟或日级历史。它不会发送付费模型探测，也不代表特定账号或中转线路的端到端可用性。直连不可用时，管理员可在模型监控页面配置独立代理。
